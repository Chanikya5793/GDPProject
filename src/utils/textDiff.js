...
// indication of which words moved, in a 320px sidebar. A student could not tell
// what the assistant was about to do, which is the entire reason this exists.
//
// Pure and dependency-free. Mirrored in mobile/utils/textDiff.ts, with a test
// on each side pinning them to the same output, the same way changePreview is
// kept honest.

/** Words of context kept either side of a change before collapsing. */
const CONTEXT = 8

/** Roughly six lines in a sidebar. Past this the diff stops helping. */
...
function tokenize(text) {
  return String(text || '').split(/(\s+|[^\p{L}\p{N}_]+)/u).filter(part => part !== undefined && part !== '')
}
...
function runs(before, after) {
  ...
  if (oldMiddle.length > MAX_TOKENS || newMiddle.length > MAX_TOKENS) {
    ...
    const glimpse = tokens => tokens.slice(0, MAX_TOKENS / 8).join('')
    ...
    const table = []
    for (let i = 0; i <= oldMiddle.length; i++) table.push(new Array(newMiddle.length + 1).fill(0))
 ...
 */
function collapse(list, context) {
  const out = []
 ...
 */
export function diffWords(before, after, options = {}) {
 ...
 */
export function diffSentence(stat) {
  const unit = count => (count === 1 ? 'word' : 'words')
