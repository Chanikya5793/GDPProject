...
import { diffSentence, diffStat, diffWords } from './textDiff'

const text = runs => runs.map(run => run.text).join('')
const kinds = runs => runs.map(run => run.type)
...
describe('parity with the mobile implementation', () => {
  it('produces the same runs for the same edit, so a diff reads the same on both', () => {
    // The fixture is shared with mobile/utils/textDiff.test.ts on purpose. If
    // one side is changed without the other, this is what says so.
