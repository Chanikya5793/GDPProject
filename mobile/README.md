...
The planner widgets are native SwiftUI, generated by
`plugins/withNativePlannerWidgets.js` after `expo-widgets`. Their source is in
`plugins/widgets/`; edit those files, not the generated `ios/` directory.
Widget changes require a new iOS binary. See [the widget guide](docs/WIDGETS.md)
for configuration, data flow, validation and device acceptance.
